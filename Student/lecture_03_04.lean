...
def is_even_len : String → Bool := comp' String Nat Bool is_even String.length
#reduce is_even_len "Hellof"
...
-- Want to not allow for two arguments (function and its argument) and instead
-- just a function applied to an argument twice
